...
`0.1.7`. Never skip or jump patch numbers (`0.1.5` → `0.1.7` is wrong),
...
gemchain guard ask-sentry
gemchain update ask-sentry <new-version> --dry-run
gemchain update ask-sentry <new-version>
```
...
3. Build passes — `bundle exec rake build` (or `gem build ask-sentry.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/sentry/version.rb` by gemchain, not by hand.
...
| `lib/ask/sentry/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Bug fix only: `0.1.5` → `0.1.6`.
- Several bug fixes ship together: one release, `0.1.5` → `0.1.6` (never
  `0.1.7`).
...
- This gem bumped because a dependency released: patch only, e.g. `0.1.6` →
  `0.1.7`, via the gemchain cascade — never hand-published.
